...
const TransactionsReport = () => {
  ...
  useEffect(() => {
    // Filter transactions based on search term, date range, year and month
    let filtered = transactions
    
    if (searchTerm) {
      const term = searchTerm.toLowerCase();
      filtered = filtered.filter(trans => 
        trans.mtoName.toLowerCase().includes(term)
      );
    }
    ...
    if (selectedYear) {
      filtered = filtered.filter(trans =>  new Date(trans.date).getFullYear().toString().includes(selectedYear));
    }
    
    if (selectedMonth && selectedYear) {
      const yearMonth = `${selectedYear}-${selectedMonth}`;
      filtered = filtered.filter(trans => trans.date.startsWith(yearMonth));
    }
    
    setFilteredTransactions(filtered);
  }, [transactions, searchTerm, startDate, endDate, selectedYear, selectedMonth]);
    ...
    <div className="flex flex-col p-4">
      ...
      <div className="mt-4">
        <Table>
          ...
          <TableBody>
            {filteredTransactions.filter(transaction=> transaction.mtoName!==null)
                                .map(transaction => (
