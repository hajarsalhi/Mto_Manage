...
import com.mtoManage.CP_mtoLedger.services.TresoProductService;
...
@Service
public class TresoProductServiceImpl implements TresoProductService {
    ...
    public void createProduct(ProductFinParams product) {
        TresoProduct tresoProduct = new TresoProduct();
        tresoProduct.setProductId(product.getProductId());
        tresoProduct.setProductName(product.getProductName());
        tresoProduct.setActive(product.getActive());
        tresoProduct.setDecote(product.getDecote());
        tresoProduct.setLibDecote(product.getLibDecote());
        tresoProduct.setEmail(product.getEmail());
        tresoProduct.setEmailReconciliation(product.getReconciliationEmails());
        tresoProduct.setSendReconciliationFile(product.getSendReconciliationReports());
        tresoProduct.setApplyHour(product.getApplyHour());
        tresoProduct.setProductDeviseFx(product.getDevise());

        productRepository.save(tresoProduct);

        //add a new  current balance record
        TresoCurrentBalance currentBalance = new TresoCurrentBalance();
        currentBalance.setProductId(product.getProductId());
        currentBalance.setCurrentBalance(BigDecimal.ZERO);
        currentBalance.setVersion(0l);
