...
import com.mtoManage.CP_mtoLedger.services.impl.EmailService;
...
public class ReconciliationReportService {
    ...
    private final TresoCompensationRepository compensationRepository;
    ...
    public void generateAndSendReport(TresoProduct product) throws IOException, MessagingException {
        ...
        // Get absolute path for reports
        String reportsBasePath = System.getProperty("user.home") + "/.mto_reports";
        Path reportDir = Paths.get(reportsBasePath, "recon_reports_" + product.getProductId());
        ...
        log.info("Generating report at: {}", filePath.toAbsolutePath());
        String s3FilePath = "recon_mto/recon_reports_" + product.getProductId() + "/" + fileShortName;

        try {
            // First check if the file exists and delete if it does
            ...
            // Write using BufferedWriter with UTF-8 encoding
            try (BufferedWriter writer = Files.newBufferedWriter(filePath, java.nio.charset.StandardCharsets.UTF_8)) {
                ...
                writer.newLine();
                log.info("Successfully wrote header to file");

                LocalDate reportDate = LocalDate.now().minusDays(1);
                BigDecimal accountBalance = writeReportContent(writer, product, reportDate);
                log.info("Successfully wrote content to file");
            ...
            // Close the writer explicitly and flush the content
            writer.close();
            
            // Verify file exists and has content before sending email
            if (Files.exists(filePath) && Files.size(filePath) > 0) {
                ...
                String[] toEmails = product.getEmailReconciliation().split(",");
                emailService.sendEmailWithAttachment(
                    toEmails,
                    "Reconciliation Report - " + product.getProductName() + " - " + reportDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")),
                    emailBody,
                    filePath
                );
                log.info("Successfully sent email with attachment for product {}", product.getProductId());
            } else {
                log.error("File is empty or doesn't exist after writing: {}", filePath);
                throw new IOException("File is empty after writing");
            }

            log.info("Reconciliation report for product {} sent successfully to {}", 
                product.getProductId(), 
                product.getEmailReconciliation());
            } catch (IOException e) {
    ...
    private BigDecimal writeReportContent(BufferedWriter writer, TresoProduct product, LocalDate reportDate) throws IOException {
        ...
        // Write transactions
        List<TresoTransactions> transactions = transactionRepository.findByProductAndDate(
            product.getProductId(), reportDate);
        for (TresoTransactions trx : transactions) {
            BigDecimal commission = trx.getAmountDH()
                .multiply(trx.getCommission())
                .divide(BigDecimal.valueOf(100));
            
            accountBalance = accountBalance.subtract(trx.getAmountDH()).subtract(commission);

            BigDecimal sentAmount =  PRODUCT_BALANCE_MAD.contains(product.getProductId()) ? 
                trx.getAmountDH():trx.getAmountDevise();
            ...
            writer.write(String.format("%s,Disbursement,%s,%s,%s,%.2f,%.2f,%.4f,%s,%.2f,%.2f",
                ...
                trx.getTransactionInternalCode(),
                trx.getSendCurrency(),
                sentAmount,
                commission,
                trx.getTauxTheorique(),
                "MAD",
                trx.getAmountDH(),
                accountBalance
            ));
        ...
        TresoBalance endBalance = balanceRepository.findByProductAndDate(
