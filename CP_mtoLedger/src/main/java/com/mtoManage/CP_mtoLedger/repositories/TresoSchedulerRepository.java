...
public interface TresoSchedulerRepository extends JpaRepository<TresoScheduler, Integer> {
    ...
    boolean existsByDateTimeAndNameAndState(
    ...
    @Query("SELECT COUNT(t) > 0 FROM TresoScheduler t WHERE CAST(t.date as date) = :date AND t.schedulerName = :name AND t.state = :state")
