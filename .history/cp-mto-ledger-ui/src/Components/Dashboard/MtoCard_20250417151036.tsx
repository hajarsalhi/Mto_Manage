...
import { ArrowDownRight, ArrowUpRight, Edit, Eye } from "lucide-react";
...
interface MtoCardProps {
  ...
  onViewMto: (mtoId: string) => void;
  onEditBalance: (mtoId: string) => void;
}
...
export function MtoCard({ 
  ...
  const change = ((balance - prevBalance) / prevBalance) * 100;
  ...
  return (
    <Card className={cn(
      "overflow-hidden transition-all duration-200 hover:shadow-soft animate-fade-in card-shine",
      riskUsagePercentage > 80 && "border-2 border-red-500"
    )}>
      ...
      <CardContent>
        ...
        <div className="mt-2 space-y-1">
          <div className="flex justify-between text-xs text-muted-foreground">
            ...
            <span>{riskUsagePercentage.toFixed(1)}%</span>
          </div>
          <Progress 
            ...
            className={cn(
              ...
              riskUsagePercentage > 60 ? "bg-yellow-100" : 
              "bg-green-100"
            )}
          />
        </div>
          ...
          <div className="mt-2 flex justify-between items-center">
            <Button 
              variant="link" 
              size="sm" 
              className="p-0 h-auto text-xs text-muted-foreground hover:bg-green-100 hover:text-green-700" 
              asChild
            >
              <a href={`/risk-value/add/${id}`}>Edit Risk Value</a>
            </Button>
            <span 
              className={cn(
