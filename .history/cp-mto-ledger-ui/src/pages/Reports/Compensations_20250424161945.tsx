...
import { Download, FileText, FileSpreadsheet, Calendar, Search, Filter } from 'lucide-react';
...
const CompensationsReport: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCompensations = async () => {
      try {
        ...
        setFilteredCompensations(response.data);
        setError(null);
      } catch (err) {
        ...
        const mockData = Array.from({ length: 20 }, (_, i) => {
          ...
          // Randomly distribute across different months and years for testing filters
          date.setMonth(Math.floor(Math.random() * 12));
          date.setFullYear(currentYear - Math.floor(Math.random() * 3));
          
          return {
            id: i + 1,
            date: date.toISOString().split('T')[0],
            mtoName: ['Western Union', 'MoneyGram', 'Ria', 'UAE Exchange'][i % 4],
            compensation: Math.round(10000 + Math.random() * 90000) / 100,
            fxRate: Math.round((9.5 + Math.random() * 1.5) * 100) / 100,
            currency: i % 2 === 0 ? 'USD' : 'EUR' as 'USD' | 'EUR'
          };
        });
        
        setCompensations(mockData);
        setFilteredCompensations(mockData);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDownloadPDF = async () => {
    try {
      setDownloading(true);
      // Replace with your actual API endpoint
      const response = await axios.get('/api/reports/compensations/pdf', {
        params: { 
          startDate, 
          endDate, 
          search: searchTerm,
      ...
      link.href = url;
      link.setAttribute('download', `compensations_report_${format(new Date(), 'yyyyMMdd')}.pdf`);
  ...
  const handleDownloadCSV = async () => {
    try {
      setDownloading(true);
      // Replace with your actual API endpoint
      const response = await axios.get('/api/reports/compensations/csv', {
        params: { 
          startDate, 
          endDate, 
          search: searchTerm,
  ...
  return (
    <div className="container mx-auto py-6">
      <h1 className="text-2xl font-bold mb-6">Compensations Report</h1>
