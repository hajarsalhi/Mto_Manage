...
import { BalanceResponse, getBalanceByProductId, editCurrentBalance } from '../../services/balance.service.ts';
...
const EditBalance: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBalanceData = async () => {
      try {
        setLoading(true);
        if (id) {
          const response = await getBalanceByProductId(parseInt(id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if(id){
        await editCurrentBalance(parseInt(id), parseFloat(correctionAmount), parseFloat(newBalance), reason);
      }
          ...
          <div className="mt-6">
            ...
            >
              {loading ? (
                ...
                <Save className="h-4 w-4 bg-green-500 text-white hover:bg-green-600" />
              )}
              Save Correction
            </Button>
          </div>
