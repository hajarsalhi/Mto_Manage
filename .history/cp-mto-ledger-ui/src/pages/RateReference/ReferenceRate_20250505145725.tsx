...
import { getRateReference, getTwoPreviousRates } from '../../services/RateReference.service.ts';
...
const ReferenceRate: React.FC = () => {
  ...
  const fetchReferenceRates = async () => {
    try {
      setLoading(true);
      const response = await getTwoPreviousRates();
  ...
  const getPreviousRate = (devise : string)=> {
    const previousRate = referenceRates
      .filter((rate) => rate.devise === devise)
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[1];
    return previousRate ? previousRate.coursVirement : undefined;
  }
    ...
    <div className="container mx-auto py-6">
      ...
      {loading ? (
        ...
        <div className="bg-white shadow-md rounded-lg p-6 text-center">
          ...
          <Button 
            onClick={handleUpdateRates}
            className="mt-4 flex items-center gap-2 mx-auto bg-green-500 text-white hover:bg-green-600"
          >
            <Edit className="h-4 w-4" />
            Update Reference Rates
          </Button>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right font-bold">Cours Virement</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {referenceRates?.slice(2).map((rate) => (
                <TableRow key={rate.id}>
                  <TableCell>{formatDate(rate.date)}</TableCell>
                  <TableCell className="font-medium">{rate.devise}</TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <span className={getRateChangeClass(rate.coursVirement, getPreviousRate(rate.devise))}>
                        {formatCurrency(rate.coursVirement)}
                      </span>
                      {getRateChangeIcon(rate.coursVirement, getPreviousRate(rate.devise))}
                    </div>
                  </TableCell>
                </TableRow>
              ))}
