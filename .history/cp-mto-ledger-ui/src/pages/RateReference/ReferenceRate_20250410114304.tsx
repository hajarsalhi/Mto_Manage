...
const ReferenceRate: React.FC = () => {
  ...
  const fetchReferenceRates = async () => {
    try {
      ...
      setReferenceRates(response.data);
      ...
      setReferenceRates(mockData as ReferenceRateData[]);
      setLastUpdated(new Date());
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="container mx-auto py-6">
                ...
                <TableRow key={rate.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <span className={getRateChangeClass(rate.rate, rate.previousRate)}>
                        {formatCurrency(rate.rate)}
                      </span>
                      {getRateChangeIcon(rate.rate, rate.previousRate)}
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="mt-6 bg-blue-50 p-4 rounded-md">
        <h2 className="text-lg font-semibold text-blue-800 mb-2">About Reference Rates</h2>
        <p className="text-sm text-blue-700">
          Reference rates are updated daily from Bank Al-Maghrib. These rates are used as the base for calculating 
          FX rates for MTOs with their respective discounts applied. The system automatically fetches the latest rates 
          each morning, but you can manually update them using the button above if needed.
        </p>
      </div>
      */}
    </div>
