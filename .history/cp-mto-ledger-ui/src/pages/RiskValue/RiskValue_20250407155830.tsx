...
const RiskValue: React.FC = () => {
  ...
  const getStateText = (dateEnd: string | null) => {
  ...
  const activeRiskValues = filteredRiskValues?.filter(rv => getStateText(rv.dateEnd )=== 'Active');
  const historyRiskValues = filteredRiskValues?.filter(rv => getStateText(rv.dateEnd ) !== 'Active');
                      ...
                      <TableRow key={riskValue.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            {getStateIcon(getStateText(riskValue.dateEnd))}
                            <span>{getStateText(riskValue.dateEnd)}</span>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
