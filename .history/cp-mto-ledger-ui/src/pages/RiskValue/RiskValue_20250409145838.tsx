...
const RiskValue: React.FC = () => {
  ...
  const activeRiskValues =  Array.isArray(filteredRiskValues) && filteredRiskValues?.filter(rv => getStateText(rv.state )=== 'Active');
  const historyRiskValues = Array.isArray(filteredRiskValues) && filteredRiskValues?.filter(rv => getStateText(rv.state) !== 'Active');
