...
export const getTwoPreviousRates = async () => {
    try {
        const response = await axios.get(`${API_URL}/getTwoLatestEurAndUsd`);
        return response.data;
    } catch (error) {
        console.error('Error fetching two previous rates:', error);
        throw error;
    }
}