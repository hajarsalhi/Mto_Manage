...
public interface TresoBalanceRepository extends JpaRepository<TresoBalance , Integer> {
    ...
    @Query("Select tb FROM TresoBalance tb WHERE tb.id.productId = :productId and DATE (tb.dateUpdate) = DATE (CURRENT_DATE - INTERVAL 1 DAY)")
    TresoBalance findJ_1BalanceByProductId(Integer productId);
