...
public class TauxBkamServiceImpl implements TauxBkamService {
    ...
    public TresoBkam updateRate(RateReferenceRequest request) {
        ...
        newRate.set(username);


        // create a Notification record 
        TresoActivationNotif tresoActivationNotif = new TresoActivationNotif();
        tresoActivationNotif.setDate(LocalDate.now());
        tresoActivationNotif.setState(false);
        tresoActivationNotif.setCreatedBy(username);

        tresoNotificationsRepository.save(tresoActivationNotif);
