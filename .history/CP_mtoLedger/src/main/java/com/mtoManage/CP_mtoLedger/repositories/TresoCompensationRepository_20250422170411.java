...
public interface TresoCompensationRepository extends JpaRepository<TresoCompensation, Integer> {
    ...
    @Query("SELECT COALESCE(SUM(tc.compensation), 0) FROM TresoCompensation tc WHERE 'DATE',tc.insertion BETWEEN :startDate AND :endDate AND tc.productId = :productId")
