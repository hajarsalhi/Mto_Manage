...
public class TresoFxRateNotifService {
    ...
    public void enableFxRateNotification(FxRateNotifRequest request) {
        ...
        if (request.getId() != null && request.getEnabled().booleanValue()) {
            // If it exists, update the existing record
            activationNotifRepository.updateNotificationStatus(request.getId(), !request.getEnabled().booleanValue());
        } else {
    ...
    public Boolean getNotificationStatus() {
        // Get the latest notification status from the repository
        TresoActivationNotif latestNotification = activationNotifRepository.findTopByOrderByDateActivationDesc(LocalDate.now());
        TresoActivationNotif tresoActivationNotif ;
        if (tresoActivationNotif != null) {
            return tresoActivationNotif.getState();
        }
        return false;
