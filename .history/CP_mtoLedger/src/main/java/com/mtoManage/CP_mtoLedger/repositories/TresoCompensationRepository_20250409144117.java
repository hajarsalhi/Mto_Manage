...
public interface TresoCompensationRepository extends JpaRepository<TresoCompensation, Integer> {
    ...
    @Query("SELECT SUM(tc.compensationAmount) FROM TresoCompensation tc WHERE tc.productId = :productId AND FUNCTION('DATE', tc.dDate) = FUNCTION('DATE', FUNCTION('DATE_SUB', CURRENT_DATE, 1))")
