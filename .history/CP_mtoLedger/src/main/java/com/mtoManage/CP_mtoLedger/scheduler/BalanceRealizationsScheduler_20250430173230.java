...
public class BalanceRealizationsScheduler {
    ...
    public void setBalanceRealizations() {
        ...
        try {
            ...
            TresoMessages message = new TresoMessages();
            message.setDate(LocalDateTime.now());
            message.setMessage("<i style=\"color:green\" class=\"fa fa-check-circle\"></i> setBalanceRealisations for " + chosenDate + " was completed successfully");
            messageRepository.save(message);
            ...
            scheduler.setSchedulerName(schedulerName);
            scheduler.setState(0);
            scheduler.setErrorType(e.getClass().getSimpleName());
            scheduler.setErrorDesc(e.getMessage().substring(0, Math.min(e.getMessage().length(), 255)));
            schedulerRepository.save(scheduler);

            TresoMessages message = new TresoMessages();
            message.setDate(LocalDateTime.now());
            message.setMessage("<i style=\"color:red\" class=\"fa fa-ban\"></i> setBalanceRealisations for " + chosenDate + " was not completed successfully");
            messageRepository.save(message);
        }
    }
    ...
    private void processDailyBalances(LocalDate chosenDate, List<Integer> bankDepositProducts) {
        ...
        for (TresoProduct product : activeProducts) {
            ...
            balance.setId(new TresoBalanceId(chosenDate, product.getProductId()));
        ...
        for (TresoProduct product : activeProducts) {
            ...
            // Calculate compensation (sum of compensated transactions)
            BigDecimal compensationJ1 = compensationRepository.calculateCompensationSumForDateRange(
                chosenDate.minusDays(1).atStartOfDay(),
                chosenDate.atStartOfDay(),
                product.getProductId()
            );
            ...
            BigDecimal computedBalance = balanceRepository.findByDateAndProductId(chosenDate, product.getProductId()).getBalance()
                                        .subtract(commissionJ1)
                                        .subtract(realisationJ1);
            ...
            if (balance != null) {
                ...
                balance.setCompensationJ1(compensationJ1);
                balance.setComputedBalance(computedBalance);
                balanceRepository.save(balance);
            }
        }
