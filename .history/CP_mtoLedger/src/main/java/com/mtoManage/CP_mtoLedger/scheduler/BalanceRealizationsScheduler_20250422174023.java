...
public class BalanceRealizationsScheduler {
    ...
    public void setBalanceRealizations() {
        ...
        try {
            ...
            TresoMessages message = new TresoMessages();
            message.setDate(LocalDateTime.now());
            message.setMessage("<i style=\"color:green\" class=\"fa fa-check\"></i> setBalanceRealisations for " + chosenDate + " was completed successfully");
            messageRepository.save(message);
    ...
    private void processDailyBalances(LocalDate chosenDate, List<Integer> bankDepositProducts) {
        ...
        for (TresoProduct product : activeProducts) {
            ...
            if (balance != null) {
                ...
                balance.setBalance(balance.getBalance().add(compensationJ1).subtract(realisationJ1).subtract(commissionJ1));
                balanceRepository.save(balance);
            }
        }
