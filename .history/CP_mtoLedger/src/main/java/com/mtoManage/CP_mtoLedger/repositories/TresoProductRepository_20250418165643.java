...
public interface TresoProductRepository extends JpaRepository<TresoProduct, Integer> {
    ...
    @Query("SELECT pp.productId FROM TresoProduct pp WHERE pp.partnerInfo.canSendBank = 1 AND (pp.partnerInfo.canSendCash IS NULL OR pp.partnerInfo.canSendCash = 0)")
