...
public class RateReferenceController {
    ...
    @PostMapping("/update")
    public ResponseEntity<?> updateRateReference(@RequestBody RateReferenceUpdateRequest request) {
        try {
    ...
    @GetMapping("/getTwoLatestEurAndUsd")
    public List<TresoBkam> getTwoLatestEurAndUsd() {
        try {
            System.out.println(rateReferenceServiceImpl.getTwoLatestEurAndUsdRates());
            return rateReferenceServiceImpl.getTwoLatestEurAndUsdRates();
