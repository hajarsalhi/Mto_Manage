...
import com.mtoManage.CP_mtoLedger.repositories.TresoTransactionsRepository;
...
public class BalanceController {
    ...
    public ResponseEntity<List<BalanceResponse>> getDayBeforeFinanceData() {
        ...
        for (TresoProduct product : products) {
            // Get the J-1 balance for each product
            TresoBalance j1Balance = balanceRepository.findJ_1BalanceByProductId(product.getProductId());
            
            // Get the J-1 compensation for each product
            BigDecimal j1Compensation = compensationRepository.findTotalJ_1CompensationByProductId(product.getProductId());
            if (j1Compensation == null) {
                j1Compensation = BigDecimal.ZERO;
            }

            BigDecimal j1Transaction = transactionRepository.findTotalJ_1TransactionByProductId(product.getProductId());
            ...
            if (j1Balance != null) {
                ...
                // For transactions, we'll use the realisationJ1 field from the balance
                // This represents the total transactions for J-1
