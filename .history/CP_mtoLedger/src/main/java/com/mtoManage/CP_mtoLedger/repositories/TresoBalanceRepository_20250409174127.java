...
public interface TresoBalanceRepository extends JpaRepository<TresoBalance , Integer> {
    ...
    @Query(value = "SELECT tb.n_balance from treso_balance tb where tb.id.productId =: productId and tb.id.date = DATE_SUB(CURRENT_DAY, INTERVAL 1 DAY)", nativeQuery = true)
