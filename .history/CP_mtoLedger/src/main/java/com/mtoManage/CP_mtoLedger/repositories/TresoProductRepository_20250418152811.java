...
public interface TresoProductRepository extends JpaRepository<TresoProduct, Integer> {
    ...
    @Query("SELECT pp.id FROM TresoProduct pp JOIN PartnerInfo pi ON pi.partnerId = pp.id WHERE pi.canSendBank = 1 AND (pi.canSendCash IS NULL OR pi.canSendCash = 0)")
