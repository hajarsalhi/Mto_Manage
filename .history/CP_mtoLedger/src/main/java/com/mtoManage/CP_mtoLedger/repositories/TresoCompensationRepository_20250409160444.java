...
public interface TresoCompensationRepository extends JpaRepository<TresoCompensation, Integer> {
    ...
    @Query(value = "SELECT SUM(tc.compensation) FROM treso_compensation tc WHERE tc.product_id = :productId AND DATE(tc.date) = DATE(DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY))")
