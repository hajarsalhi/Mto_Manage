...
import com.mtoManage.CP_mtoLedger.dto.NewProductResponse;
...
import com.mtoManage.CP_mtoLedger.repositories.ProductRepository;
...
import com.mtoManage.CP_mtoLedger.services.impl.TresoProductServiceImpl;
...
public class ProductController {
    ...
    @Autowired
    private TresoProductServiceImpl productServiceImpl;
    ...
    public ResponseEntity<List<TresoProduct>> getMoneyServProducts() {
        List<TresoProduct> products = productServiceImpl.getAllProductsList();
        return ResponseEntity.ok(products);
    }

    @GetMapping("/newProducts")
    public ResponseEntity<List<NewProductResponse>> getAllNewProducts(){
        List<Product> newProducts =   productRepository.findAll();
        List<NewProductResponse> newProductResponse = new ArrayList<>();
        for (Product product : newProducts) {
            NewProductResponse response = new NewProductResponse();
            response.setId(product.getId());
            response.setName(product.getName());
            newProductResponse.add(response);
        }
        return ResponseEntity.ok(newProductResponse);
    }
    ...
    @GetMapping("/getTresoProducts")
    public ResponseEntity<List<GetProductResponse>> getAllTresoProducts() {
        List<GetProductResponse> products = productServiceImpl.getAllProducts();
        return ResponseEntity.ok(products);
    }

    @GetMapping("/{id}/finParams")
    ...
    @PutMapping("/{id}/finParams")
    public ResponseEntity<TresoProduct> updateProduct(@PathVariable Integer id, @RequestBody ProductFinParams product) {
        productServiceImpl.updateProduct(id, product);
        return ResponseEntity.ok().build();
    }

    @PutMapping("newProduct")
    public ResponseEntity<?> createnewProduct(@RequestBody ProductFinParams product) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("error", "Error creating Product: " + e.getMessage()));
        }
    }
