...
public interface PartnerProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query("SELECT pp.productId FROM Product pp JOIN PartnerInfo pi ON pp.productId = pi.id WHERE pi.canSendBank = true AND (pi.canSendCash IS NULL OR pi.canSendCash = false)")
