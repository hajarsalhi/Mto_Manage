...
public interface TresoCompensationRepository extends JpaRepository<TresoCompensation, Integer> {
    ...
    @Query("Select tc from TresoCompensation tc where tc.productId = :productId and tc.date = :date")
    TresoCommissionRepository findByProductAndDate(
        @Param("productId") Integer productId,
        @Param("date") LocalDate date
    );

    @Query("SELECT tc FROM TresoCompensation tc WHERE tc.insertion BETWEEN :startDate AND :endDate AND tc.productId = :productId")
    List<TresoCompensation> findCompensationsBetweenDates(
        @Param("startDate") LocalDateTime startDate,
        @Param("endDate") LocalDateTime endDate,
        @Param("productId") Integer productId
    );
