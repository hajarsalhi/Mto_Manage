...
public class BalanceController {
    ...
    public ResponseEntity<List<CurrentBalanceRequest>> getCurrentBalances() {
        List<CurrentBalanceRequest> responses = new ArrayList<>();
        
        // Get all products
        List<TresoProduct> products = productRepository.findAll();
        
        for (TresoProduct product : products) {
            // Get the most recent current balance for each product
            TresoCurrentBalance recentCurrentBalance = currentBalanceRepository.findByProductId(product.getProductId());
            
            CurrentBalanceRequest response = new CurrentBalanceRequest();
            response.setProductName(product.getProductName());
            response.setStatus(recentCurrentBalance != null ? "AVAILABLE" : "NOT_AVAILABLE");
            
            if (recentCurrentBalance != null) {
                response.setBalance(recentCurrentBalance.getCurrentBalance());
                response.setPrevBalance(currentBalanceRepository.findPrevBalanceByProductId(product.getProductId()));
                response.setLastTransactionTime(transactionRepository.findLastTransactionDate(null).toString());
            }
