...
public interface TresoCompensationRepository extends JpaRepository<TresoCompensation, Integer> {
    ...
    @Query(value = "SELECT SUM(tc.n_compensation) FROM treso_compensation tc WHERE tc.n_product_id = :productId AND DATE(tc.d_date) = DATE(DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY))", nativeQuery = true)
