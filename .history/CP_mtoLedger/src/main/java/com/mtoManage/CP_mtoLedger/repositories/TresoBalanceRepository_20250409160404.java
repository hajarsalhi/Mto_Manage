...
public interface TresoBalanceRepository extends JpaRepository<TresoBalance , Integer> {
    ...
    @Query(value = "SELECT * FROM treso_balance tb WHERE tb.product_id = :productId AND DATE(tb.d_date_update) = DATE(DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY))", nativeQuery = true)
