...
public interface PartnerProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query("SELECT pp.id FROM Product pp JOIN PartnerInfo pi ON pi.nPartnerId = pp.id WHERE pi.canSendBank = true AND (pi.canSendCash IS NULL OR pi.canSendCash = false)")
