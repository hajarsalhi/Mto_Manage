...
public interface TresoTransactionsRepository extends JpaRepository<TresoTransactions, Integer> {
    ...
    @Query("SELECT SUM(t.amount) FROM TresoTransactions t WHERE t.productId = :productId AND FUNCTION('DATE', t.date) = FUNCTION('DATE', FUNCTION('DATE_SUB', CURRENT_DATE, 1))")
