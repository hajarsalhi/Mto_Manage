...
public class CompensationController {
    ...
    @PostMapping("/upload")
    public ResponseEntity<UploadResponse> uploadCSV(@RequestParam("file") MultipartFile file, @PathVariable("fileName") String fileName) {
        try {
            if (file.isEmpty()) {
    ...
    public ResponseEntity<List<CompensationData>> getValidCompensations() {
        ...
        for(TresoCompensation compensation : validCompensations) {
            TresoProduct product = tresoProductRepository.findById(compensation.getProductId()).orElse(null);
            CompensationData compensationData = new CompensationData().builder()
                .id(1)
                .date(LocalDate.now())
                .mtoName(compensation.getProductName())
                .compensation(compensation.getCompensation())
                .fxRate(compensation.getTaux())
                .currency(product != null ? product.getProductDeviseFx() : null)
                .build();
