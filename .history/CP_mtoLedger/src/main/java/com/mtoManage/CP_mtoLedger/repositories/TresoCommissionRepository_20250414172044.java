...
public interface TresoCommissionRepository extends JpaRepository<TresoCommission, Integer> {
    ...
    @Query(value = "SELECT * FROM dbo.Treso_Commission WHERE Taux_commission IS NOT NULL", nativeQuery = true)
    Optional<List<TresoCommission>> findCommissionRate();

    @Query(value = "SELECT Taux_commission FROM dbo.Treso_Commission WHERE Product_id = :productId", nativeQuery = true)
