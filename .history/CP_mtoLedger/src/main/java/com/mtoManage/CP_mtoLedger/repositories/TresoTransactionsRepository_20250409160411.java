...
public interface TresoTransactionsRepository extends JpaRepository<TresoTransactions, Integer> {
    ...
    @Query(value = "SELECT SUM(t.amount) FROM treso_transactions t WHERE t.product_id = :productId AND DATE(t.date) = DATE(DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY))", nativeQuery = true)
